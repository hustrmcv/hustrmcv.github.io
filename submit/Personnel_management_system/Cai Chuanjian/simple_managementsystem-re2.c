/*simple_managementsystem.c——简易人员管理系统 2.0*/
#include<stdio.h>
#include<stdlib.h>
#include<string.h>
#define MAX 20
...
int usernumber=0;                 //全局变量，初始化用户数量为0


void Menu();                      //说明提示信息，对人员信息的各种操作

void bc();                       //保存添加和修改后的人员信息，用在Add()，Modify()函数中，文件写模式
void bcs(struct admin *num) ;    //保存添加的用户信息，用在reg()函数中，在文件末尾添加内容

void Back();                     //提示返回主菜单（无实际内容）

int dq(struct staff per[]);      //读取，处理结构体数组，返回值为当前元素个数（人员数量）
int dqs(struct admin num[]);     //同上
...
int main(void)
{
    ...
    if((fpb=fopen("staff_b.txt","rb"))==NULL)
    {
    	...
    	if(yesorno=='y'||yesorno=='Y')
    	{
    		...
    		fprintf(fp,"%s%10s%10s%15s%15s%20s%15s\n","组别","姓名","专业年级","电话","qq","邮箱","学号");
...
void Putm()
{
	...
	for(i=0;i<Number;i++)
	{
		...
		{
			printf("%s%10s%10s%15s%15s%20s%15s\n","组别","姓名","专业年级","电话","qq","邮箱","学号");
			printf("%s%10s%10s%15s%15s%20s%15s\n",per[i].sgroup, per[i].sname, per[i].smage, per[i].stele, per[i].sqq, per[i].semail, per[i].sno);
			count++;
		}
	}
...
/*	for(i=0;i<Number;i++)
    ...
    if(count>0)
    {
        ...
        printf("是否要删除？(确认请输入y/Y,取消请输入非y/Y的任意字符)\n");

        char yorn;

        while(scanf("%c",&yorn)==1&&(yorn=='y'||yorn=='Y'))
        {
            for(i=0;i<Number;i++)
            {
                if(strcmp(name,per[i].sname)==0)
...
void Reg()//注册 //缺点：存在长度限制，空格问题
{
	int i=0;
	int count=0;

	printf("[Register...]\n");
	printf("（注意用户名和密码长度均不得超过20个字符,且不要使用空格）\n\n");
	printf("用户名：");
	scanf("%s",easy.user);
	fflush(stdin);
	printf("密码：");
...
void Modify()
{
    ...
    if(count>0)
    {
        ...
        printf("是否要修改？(确认请输入y/Y,取消请输入非y/Y的任意字符)\n");
